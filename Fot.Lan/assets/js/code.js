...
var assessmentSelectionTemplate = null;
...
$(function () {
    ...
    $.get(loader_url).done(function (data) {
        ...
        if (bundle.flagsuccess === true) {
            ...
            if (bundle.current_assessment_index < 0 && bundle.allow_assessment_selection === false) {

                bundle.current_assessment_index = 0;

            }

            if (bundle.candidate_photo !== null) {

                $("#imgCandidate").prop("src", "data:image/png;base64, " + bundle.candidate_photo);

            }

           

            if (bundle.current_assessment_index >= 0 && bundle.assessments[bundle.current_assessment_index].started) {
    ...
    assessmentSelectionTemplate = Handlebars.compile($("#assessmentSelection").html());
...
var firstPage = (function () {
    ...
    var moveNext = function () {
        ...
        if (bundle.allow_assessment_selection) {

            if (bundle.current_assessment_index < 0) {

                var tempIndex = getIndexIfSingleAssessment();
                if (tempIndex >= 0) {

                    bundle.current_assessment_index = tempIndex;

                    html = instructionTemplate(bundle.assessments[bundle.current_assessment_index]);

                } else {

                    var dataContext = { assessments_table: getAssessmentsTable() };

                    html = assessmentSelectionTemplate(dataContext);
                }
    ...
    var selectAssessment = function(selectedId) {

        for (var i = 0; i < bundle.assessments.length; i++) {

            if (bundle.assessments[i].assessment_id === selectedId) {

                bundle.current_assessment_index = i;
    ...
    var getIndexIfSingleAssessment = function() {

        var total = 0;
        var tempIndex = -1;
        for (var i = 0; i < bundle.assessments.length; i++) {

            if (bundle.assessments[i].completed === false) {
    ...
    var getAssessmentsTable = function() {

        var prefix = '<table class="table table-bordered table-hover table-striped"><tbody><tr><th>Assessment</th><th style="width: 140px;">&nbsp;</th></tr>';
        ...
        for (var i = 0; i < bundle.assessments.length; i++) {
            ...
            if (bundle.assessments[i].completed === false) {
                ...
                var temp = "<tr><td>" + bundle.assessments[i].assessment_name + "</td><td><button class='btn btn-default navButton' onclick='firstPage.selectAssessment(" + bundle.assessments[i].assessment_id + ")'>Start Test</button></td> </tr>";
...
var starterPage = (function () {
    ...
    var startTest = function () {
        ...
        if (bundle.assessments[bundle.current_assessment_index].show_calculator === true) {
...
var essayStarterPage = (function () {
    ...
    var startTest = function () {
        ...
        if (candidate_response.length > 0) {

            essayPage.countWords(candidate_response);
        }

        $("#editor").kendoEditor({
            change: function () {

                essayPage.countWords(this.value());
            },
...
var assessmentPage = (function () {
           ...
           if(minutes > 0) minutes--;
            ...
            for (var j = 0; j < tempBundle.assessments.length; j++) {
                ...
                if (tempBundle.assessments[j].assessment_type === 'MCQ') {
                    ...
                    for (var x = 0; x < tempBundle.assessments[j].essays.length; x++) {

                        tempBundle.assessments[j].essays[x].topic = '';
            ...
            $.ajax({
                ...
                .done(function (result) {
                    ...
                    $("time#timeSaved").timeago("update", new Date().toISOString());
    ...
    function progressAssessment() {
        ...
        $("#divShowCalculator").hide(); // remove for unproctored.
        ...
        bundle.assessments[bundle.current_assessment_index].completed = true;

        var html = "";

        if (bundle.allow_assessment_selection) {

            if (TotalAssessmentsPending() === 0) {

                $("body").off('keyup');

                //end assessment session

                 html = assessmentEndTemplate(bundle);
                ...
                //progress assessment 
                bundle.current_assessment_index = -1;
    ...
    function TotalAssessmentsPending() {
...
var essayPage = (function () {
    ...
    var countWords = function(dataStr) {
        ...
        $("#txtWords").text(words + (words === 1 ? " WORD" : " WORDS"));
    }
...
var assessmentEndPage = (function () {
    ...
    var submitAssessment = function () {
        ...
        .done(function (result) {
            ...
            if (result.Succeeded === true) {
                ...
                $("#divStatus").html('<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Assessment submitted successfully.</div>');

                $("#divProgress").hide();
                if (bundle.show_results_on_submit === true && result.resultList.length > 0) {

                 $("#divResult").show();
                 $("#divResult").html(getScoreTable(result.resultList));
                ...
                if (bundle.min_aggregate_score !== null) {
                    ...
                    $("#lblPassFail").show();
                    $("#lblPassFail").html(totalScore >= bundle.min_aggregate_score ? "<strong style='color: green; font-size: 18px;'>Congratulations. You were successful at this stage.</strong>" : "<strong style='color: red; font-size: 18px;'>Sorry, your attempt was not successful.</strong>");

                }


                if (bundle.show_feedback) {
    ...
    function getScoreTable(results) {
        ...
        var prefix = '<table class="table table-bordered table-hover table-striped"><tbody><tr><th>Assessment</th><th style="width: 140px;">Score</th></tr>';
