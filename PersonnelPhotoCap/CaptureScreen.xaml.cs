...
namespace PhotoCap
{
    ...
    public partial class CaptureScreen : UserControl
    {
        ...
        {
            lblCandidateName.Text = MainWindow.SelectedStaff.Firstname + " " + MainWindow.SelectedStaff.Lastname;
        ...
        private async void DoSave()
        {
            using (var service = new ImageCaptureServiceClient())
            {
                try
                {
                    ...
                    var result = await service.UpdateStaffAsync(MainWindow.SelectedStaff.AdminId, currentImageBytes, MainWindow.username, MainWindow.password, MainWindow.CapturePhase);

                    if (result.Succeeded)
                    {

                        MainWindow.staffList.Remove(MainWindow.SelectedStaff);

                        EnableControls(true);
                        progressRing.IsActive = false;


                        ShowInfo("Personnel details updated successfully.");

                        MessageBox.DisplayMessage("Info", "Personnel details updated successfully.", MainWindow.Instance);
