...
{
    public class PartnerAssessmentService : ServiceBase
    {
        ...
        public List<AssessmentViewModel> GetAssessments(string searchTerm, int startRow, int maxRows)
        {
            ...
            {
                query =
                    Assessments.Where(x => x.OwnerPartnerId == currentAdmin.PartnerId)
                                   ...
                                   x => new AssessmentViewModel
                                       {
                                           ...
                                           IsValid =
                                               x.AssessmentType == AssessmentType.MCQ
                                                   ? x.AssessmentQuestions.Any() &&
                                                     !x.AssessmentQuestions.Any(
                                                         y =>
                                                         y.AssessmentAnswers.Count() < 2 &&
                                                         !y.AssessmentAnswers.Any(t => t.IsCorrect))
                                                   : x.EssayTopics.Any()
                                       });
            }
            ...
            else
            {
                ...
                query = Context.AuthorAssignedAssessments.Where(x => x.AdminId == currentAdmin.AdminId)
                               .Select(x => new AssessmentViewModel
                                   {
                                       ...
                                       IsValid =
                                           x.Assessment.AssessmentType == AssessmentType.MCQ
                                               ? x.Assessment.AssessmentQuestions.Any() &&
                                                 !x.Assessment.AssessmentQuestions.Any(
                                                     y =>
                                                     y.AssessmentAnswers.Count() < 2 ||
                                                     !y.AssessmentAnswers.Any(t => t.IsCorrect))
                                               : x.Assessment.EssayTopics.Any()
                                   }).OrderByDescending(x => x.AssessmentId);
            }
        ...
        public int Count(string searchTerm)
        {
                ...
                if (currentAdmin.HasUsersAccess)
                {
                    return string.IsNullOrWhiteSpace(searchTerm) ? Context.Assessments.Count(x => x.OwnerPartnerId == currentAdmin.PartnerId) : Context.Assessments.Count(x => x.OwnerPartnerId == currentAdmin.PartnerId && x.Name.ToLower().Contains(searchTerm.ToLower()));
                }
                else
                {
        ...
        public AppMessage Add(Assessment item)
        {
            try
            {
                ...
                if (!currentAdmin.HasUsersAccess && currentAdmin.CanAuthor)
                {
        ...
        public AppMessage Update(Assessment item)
        {
            try
            {
                ...
                return new AppMessage { IsDone = true, Message = "Updated assessment successfully.", Status = MessageStatus.Success };
            }
        ...
        {
            return
                Assessments.Where(x => x.AssessmentBundleEntries.Any(y => y.BundleId == BundleId) == false)
                           .Select(x => new AssessmentViewModel
                           {
                               AssessmentId = x.AssessmentId,
                               Name = x.Name
        ...
        public List<AssessmentViewModel> GetAssignedAndPartnerAssessmentsNotInBundle(int BundleId)
        {
            var currentAdmin = new AdminUserService().GetCurrentAdmin();
            ...
            var partnerList =
                Assessments.Where(
                    x =>
                    x.AssessmentBundleEntries.Any(y => y.BundleId == BundleId) == false &&
                    x.OwnerPartnerId == currentAdmin.PartnerId).Select(x => new AssessmentViewModel
                    {
                        AssessmentId = x.AssessmentId,
                        Name = x.Name
                    }).ToList();
            ...
            var assignedList =
                Context.PartnerAssignedAssessments.Where(
                    x =>
                    x.Assessment.AssessmentBundleEntries.Any(y => y.BundleId == BundleId) == false &&
                    x.PartnerId == currentAdmin.PartnerId).Select(x => new AssessmentViewModel
                    {
                        AssessmentId = x.Assessment.AssessmentId,
                        Name = x.Assessment.Name
                    }).ToList();
        ...
        public List<AssessmentViewModel> GetMCQAssessmentsInBundle(int CampaignId)
        {
            ...
            var campaign = new PartnerCampaignService().GetCampaign(CampaignId);

            return
                Context.AssessmentBundleEntries.Where(
                    x => x.BundleId == campaign.BundleId && x.Assessment.AssessmentType == AssessmentType.MCQ /*&& x.Assessment.OwnerPartnerId == currentAdmin.PartnerId */).Select(
                        x =>
