...
namespace Fot.Admin.Services
{
    ...
    public class AssessmentQuestionService : ServiceBase
    {
        ...
        {
            AssessmentQuestion question = Questions.FirstOrDefault(x => x.QuestionId == QuestionId);

            if (question != null)
            {
                new AuthorAssignedAssessmentService().CheckAuthorAccess(question.AssessmentId);
            }
        ...
        public List<QuestionViewModel> GetQuestions(int AssessmentId, int ValidEntry, int startRow, int maxRows)
        {
            ...
            IEnumerable<QuestionViewModel> query =
                ctx.AssessmentQuestions.Where(x => x.AssessmentId == AssessmentId).Select(x => new QuestionViewModel
                    {
        ...
        public bool IsValidAfterDelete(int QuestionId)
        {
            ...
            else
            {
                ...
                List<AssessmentOutputConfig> configList =
                    Context.AssessmentOutputConfigs.Where(x => x.AssessmentId == assessment.AssessmentId).ToList();


                foreach (AssessmentOutputConfig config in configList)
                {
                    AssessmentOutputConfig temp = config;
                    if (
        ...
        public List<AssessmentQuestion> GetQuestionsForCampaignReports(int AssessmentId)
        {
            return
                Questions.Where(x => x.AssessmentId == AssessmentId)
                         .Include(x => x.AssessmentTopic)
                         .Include(x => x.QuestionDifficultyLevel)
                         .ToList();
        }
