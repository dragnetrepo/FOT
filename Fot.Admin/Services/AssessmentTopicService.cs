...
namespace Fot.Admin.Services
{
    public class AssessmentTopicService : ServiceBase
    {
        ...
        public List<AssessmentTopic> GetTopics(int AssessmentId, int startRow, int maxRows)
        {
            IEnumerable<AssessmentTopic> query =
                Topics.Where(x => x.AssessmentId == AssessmentId).OrderByDescending(x => x.TopicId);
        ...
        {
            return Topics.Count(x => x.AssessmentId == AssessmentId);
        }
        ...
        {
            return
                Context.AssessmentTopics.Any(
                    x => x.AssessmentId == assessmentId && x.Topic.Equals(Topic) && x.TopicId != TopicId);
        }
        ...
        public AppMessage Add(AssessmentTopic item)
        {
            if (Exists(item.Topic, item.AssessmentId))
            {
                return new AppMessage()
                    {
                        IsDone = false,
                        Message = "A topic already exists with specified name.",
            ...
            try
            {
                ...
                return new AppMessage()
                    {
                        IsDone = true,
                        Message = "Added assessment topic successfully.",
        ...
        public AppMessage Update(AssessmentTopic item)
        {
            ...
            try
            {
                ...
                return new AppMessage()
                    {
                        IsDone = true,
                        Message = "Updated assessment topic successfully.",
                        Status = MessageStatus.Success
                    };
            }
