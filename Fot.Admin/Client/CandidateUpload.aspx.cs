...
namespace Fot.Admin.Client
{
    ...
    public partial class CandidateUpload : System.Web.UI.Page
    {
        ...
        public void ProcessFile()
        {
            ...
            int totalPossible = -1;
            bool checkTotalPossible = false;

            if (campaign.IsUnproctored)
            {
                totalPossible = GetTotalPossible();
                checkTotalPossible = true;
                ...
                try
                {
                    ...
                    var app = candidateService.Add(candidate);
                        ...
                        if (app.IsDone)
                        {
                            ...
                            {
                                totalSucceeded++;
                                if (checkTotalPossible)
                                {
                                    if (totalSucceeded == totalPossible)
                                    {
                                        issuesList.Add(new UploadTempViewModel
                                            {
                                                RowNumber = 0,
                                                ClientUniqueId = string.Empty,
                                                Email =  string.Empty,
                                                ...
                                                Issue =
                                                    "Maximum number of possible candidates have been added. Wallet has to be funded to add more candidates."
                                            });
                                        errorCount += workSheet.Dimension.End.Row - rowIndex;
                                        break;
                                    }
                                }
                            }
