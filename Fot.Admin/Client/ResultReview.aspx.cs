   ...
   [PrincipalPermission(SecurityAction.Demand, Role = RoleModel.Result)]
    public partial class ResultReview : System.Web.UI.Page
    {
        ...
        public bool IsCorrect(int QuestionId, List<int> optionsChosen)
        {
            ...
            var tempChosen = optionsChosen.ToArray();
            Array.Sort(tempChosen);

            var tempReal = realOptions.Where(x => x.IsCorrect).Select(x => x.AnswerId).ToArray();
            ...
            if (realOptions.Count == 1)
            {
                ...
                if (tempChosen[0] == realOptions[0].AnswerId)
                {
                    correct = true;
                }
                else
            ...
            else
            {
                ...
                if (tempChosen.Length == tempReal.Length)
                {
                    for (int i = 0; i < tempChosen.Length; i++)
                    {
                        if (tempChosen[i] != tempReal[i])
                        {
                            correct = false;
                        }
                    }
        ...
        public string GetOptions(int QuestionId, List<int> optionsChosen, bool correct)
        {
            ...
            foreach (var item in realOptions)
            {
               ...
               // var param1 = ""; // correct && item.IsCorrect && optionsChosen.Any(x => x == item.AnswerId) ? "border: 2px solid green;" : "border: 2px solid #ff0000;";
    ...
    <div style='padding: 10px; border: 1px solid #f0f0f0; margin-top: 40px; margin-left: 50px; width: 1040px;'>
        ...
        <table style='width: 1020px;'>
            ...
            if (resultOptions != null)
            {
                ...
                var questionList = new PartnerAssessmentQuestionService().GetQuestionsAndOptions(resultOptions[0].AssessmentId);
